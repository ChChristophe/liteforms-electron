...
| 24 | `28cc967` | **Function calling + OpenClaw + audio utils** | ✅ | 9 | **fait 17/09** (function calling, détail §13) — routes Next **tournent dans le standalone** ✓ ; `audioUtils` ≈ `audioPlayback` existant ; tools réinjectés dans `googleLive`/`openAiRealtime` + `ChatPanel`/holo ; `openclawGatewayToken`. **19/09 : `openclaw_web_search` désormais porté** (le token local est résolu au boot) — route `POST /api/functions/openclaw_web_search`, catalogue + registre (dép `searchWeb` injectable), câblage `ChatPanel` ; détail + tests : `docs/porting/openclaw-web-search-audit.md` |
...
- [x] ~~Port `openclaw_web_search` (web `28cc967`, ex-volontairement exclu)~~ **FAIT (19/09/2026, validé terrain 19/09)** : route `POST /api/functions/openclaw_web_search` (token résolu côté serveur depuis `provider-credentials.json` clé `openclaw`, jamais loggué/renvoyé ; replis `body.token`/`OPENCLAW_GATEWAY_TOKEN`), définition + instruction au catalogue unique, dép `searchWeb` injectable dans le registre, câblage `ChatPanel` (token via `resolveProviderCredential`). Détail + tests : `docs/porting/openclaw-web-search-audit.md`.
...
- [x] ~~Auto-détection locale du token OpenClaw (`POC.md` §7.1) : supprimer la saisie manuelle.~~ **FAIT (19/09/2026, validé terrain 19/09)** : `electron/openclaw/gatewayToken.ts` (env → CLI → fichiers env → config littérale), écrit au boot dans `<userData>/config/provider-credentials.json` clé `openclaw`. Chemins réels : `~/.openclaw/openclaw.json` (+ `.json5`/`.jsonc`), `$OPENCLAW_CONFIG_PATH`, `$OPENCLAW_STATE_DIR`, env `~/.openclaw/.env` / `~/.config/openclaw/gateway.env` ; binaires `%APPDATA%\npm\openclaw.cmd` (Windows) et `/usr/local/bin`, `/usr/bin`, `~/.local/bin`, `~/.npm-global/bin` (Linux). **Constat** : l'OpenClaw 2026.7.1-2 renvoie `__OPENCLAW_REDACTED__` au CLI → la config littérale devient la source effective. Jamais loggué/servi. Détail + tests : `docs/porting/openclaw-token-audit.md`.
...
## 17. Post-mortem — vitesse de la voix pilotée par le Mobile (19/09/2026, validé terrain)

* **Contrat d'abord** : ajout des blocs de vitesse `providers.llm.speed` / `providers.tts.speed` dans `protocol/DEVICE_API.md` — avant toute implémentation. **La vitesse suit la voix réellement utilisée** : voice du provider TTS → `tts.speed` (plage par provider : openai `[0.25,4]`, elevenlabs `[0.7,1.2]`) ; `llm.provider` realtime → `llm.speed` (`openai-realtime` `[0.25,1.5]`, `google-live` sans vitesse). `null` = défaut provider ; valeur hors plage / slot-provider non supporté → ignorée avec warning (`providers.tts.speed ignored` / `providers.llm.speed ignored`) côté appliance.
* **Mobile** : la vitesse suit la voix — champ « Vitesse TTS » pour les providers TTS à plage (`openai` `[0.25,4]`, `elevenlabs` `[0.7,1.2]`) et « Vitesse de la voix » pour le LLM realtime `openai-realtime` (`[0.25,1.5]`) ; les plages sont portées par le catalogue (`speedRange`, référence unique des bornes, résolue par l'UI et le validateur), la validation clampe par provider, le wire porte `tts.speed` / `llm.speed` (`google-live` et LLM non-realtime sans champ). Tests verts (275).
* **Electron** : réglage desktop #4 (`OnboardingModal`) conservé et **enrichi** — l'appliance consomme désormais les deux champs (`pocConfig` validation par plage + warnings, `pocClient` mapping `tts.speed` → `TtsConfig.speed` et `llm.speed` → `realtimeVoice.speed`, `lib/speech/openAiRealtime.ts` envoie `session.audio.output.speed` **seulement si défini**), donc **chaque push téléphone écrase le réglage local** (comportement voulu). Champ desktop realtime `Vitesse de la voix (0.25 - 1.5)` ajouté pour `openai-realtime` (`google-live` sans champ). Au passage : commentaire de la borne `TTS_MAX_CONCURRENT_SYNTH = 2` (#21) et **fix racine du cleanup d'abort** (plus de synthèse fantôme après échec du stream LLM, test à l'appui).
* **Leçon** : une fonction de config ne se porte pas d'un seul côté — si le Mobile est source de vérité, le champ doit d'abord entrer au **contrat**, puis être implémenté des deux côtés (le réglage desktop seul n'est qu'un défaut). Même famille que la leçon §14 (« vérifier que la donnée arrive **et** est consommée »).
* **Validé terrain 19/09/2026** : la vitesse de la voix **realtime** (`openai-realtime` → `session.audio.output.speed`) est effective sur l'appliance ; le **token OpenClaw** est auto-découvert au boot (`via config`, écrit comme credential `openclaw`) ; l'outil **`openclaw_web_search`** est appelé par **openai-realtime** et interroge le gateway local (preuves : logs `realtime tool-call/tool-result name=openclaw_web_search` + `[openclaw] gateway token … via config`). Champ token manuel (parité Web) ajouté en Settings (`074aff8`) pour le cas où l'auto-découverte échoue. Commits : Electron `df47118`/`58d683c`/`ca11d3f`/`7663f48`/`d363486`/`074aff8`, Mobile `a6b7c5d`/`a45dde8`. Audit : `docs/porting/jarvis-openclaw-session-tts-speed-audit.md` (Electron) + `docs/porting/tts-speed-audit.md` (Mobile).
